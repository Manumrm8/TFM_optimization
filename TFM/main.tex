...
\begin{itemize}
    ...
    Si se intentara encontrar la solución exacta, sería computacionalmente muy costoso, y la naturaleza del algoritmo hace que la solución sea dependiente de los grupos iniciales (generados aleatoriamente), pudiendo caer una y otra vez en los mismos \textbf{óptimos locales} (ver en \ref{def:optimo_local}).
    ...
    \item \textbf{Selección de variables:} En la construcción de modelos de predicción, solemos encontrarnos con un número elevado de variables, por lo que definir el modelo en base a todas ellas no es factible. Mediante metaheurísticos podemos 
...
el \textbf{Problema de la Mochila}, mencionado en los casos discretos;
...
Para encontrar la solución óptima, en estos problemas de localización también se deben considerar restricciones prácticas, como la capacidad de cada centro, los límites presupuestarios o la necesidad de cubrir un mínimo de demanda.
...
\begin{itemize}
    ...
    \item \textbf{Equilibrio:} Si se construye un centro, tiene que utilizarse. No puede ocurrir que un centro quede en desuso. Por tanto, otro objetivo será que la diferencia entre los más usados y los menos sea la mínima posible, quedando las cargas bien repartidas.
\end{itemize}
...
\begin{equation}
    ...
    \label{eq:f3}
\end{equation}
La función $f_3$ minimiza la diferencia entre la instalación con más carga y la que menos. Es el objetivo más estricto en términos de \textbf{equidad operativa}, ya que busca que todas las instalaciones tengan una carga de trabajo lo más homogénea posible.
...
El funcionamiento de la búsqueda local en problemas monoobjetivo es mucho más sencillo de definir, principalmente se mejora en base a una única función objetivo, y nunca al realizar una búsqueda local se puede dar el caso de que la solución empeore. En Multiobjetivo es más delicado,
...
\begin{itemize}
    \item \textbf{Flexibilidad de adaptación}: El \textit{GRASP} es un método relativamente sencillo de codificar, pero con una amplia variedad de posibilidades
...
El algoritmo \textit{Greedy Randomizado} sigue la siguiente secuencia:  
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Se evalúan los centros candidatos no seleccionados, calculando el valor de la función objetivo $f_1$ (véase \ref{eq:f1}). Se utiliza exclusivamente $f_1$ con el fin de evitar una complejidad excesiva en esta fase, y porque es la función objetivo que puede generar soluciones más realistas y balanceadas.
        \item \textbf{Lista Restringida de Candidatos (RCL):} En lugar de escoger siempre el mejor candidato, se construye una lista con aquellos cuyos valores se encuentran próximos a él. El parámetro $\alpha$ regula el nivel de aleatoriedad:
        \begin{itemize}
...
Tras cada búsqueda local, la solución resultante se compara con el Frente de Pareto actual. Si la solución está dominada, se descarta. En caso contrario, se eliminan las soluciones del frente que resulten dominadas por la nueva, y finalmente esta se incorpora al conjunto de soluciones. De este modo, únicamente se conservan las mejores soluciones no dominadas entre sí.

Posteriormente, se ejecuta una búsqueda local direccional con el propósito de diversificar el proceso hacia regiones menos exploradas del espacio de soluciones, en las que puede resultar más prometedor encontrar nuevos óptimos.  

Las búsquedas direccionales consideradas son:  

\begin{itemize}
    \item \textbf{Búsqueda local direccional respecto a $f_1$ \ref{eq:f1}:} Minimiza la distancia máxima entre los centros seleccionados y los elementos de la población.
    \item \textbf{Búsqueda local direccional respecto a $f_2$ \ref{eq:f2}:} Reduce la carga máxima de clientes asignados a un centro.
    \item \textbf{Búsqueda local direccional respecto a $f_3$ \ref{eq:f3}:} Disminuye la diferencia de cargas entre el centro más demandado y el menos utilizado.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Con probabilidad $1 - \beta$, se selecciona una acción de forma probabilística, ponderada según su recompensa esperada (explotación). Aunque las acciones con mayor recompensa esperada tienen más probabilidad de ser elegidas, no se garantiza su elección determinista.
    \end{itemize}

    Para evitar que acciones con recompensas extremas dominen completamente la elección o sean injustamente descartadas, se aplica la función \textit{softmax} para normalizar los valores esperados:
    \begin{equation}
        \sigma(z)_i = \frac{e^{z_i - \max(z)}}{\sum_{j=1}^{K} e^{z_j - \max(z)}}
    \end{equation}
    Siendo $z_i$ el valor esperado de recompensa para la acción $i$.
...
es la búsqueda local, y en el bucle que inicia en la línea 7 y termina en la 20, el \textit{Multi-Armed Bandit} itera seleccionando una búsqueda local direccional
y mejorando la solución $X^{(2)}$. Este bucle se repite mientras se sigan encontrando soluciones no dominadas para el Frente de Pareto.


Con esta representación se puede ver de una manera más sencilla el funcionamiento del algoritmo implementado en Python. Tanto los datos, Frentes de Pareto encontrados, soluciones, como el código del algoritmo, se encuentran en el repositorio de GitHub: \url{https://github.com/Manumrm8/TFM_optimization.git}.
...
El aumento de rendimiento dependerá del procesador utilizado para ejecutar el código, es decir, a más núcleos habrá una mayor mejora.
...
todas las instancias y se obtiene el Frente de Pareto común.
...
La diferencia radica en que en la primera, \textit{WorkSpace 1000\_50\_5}, el valor de $k$ es 5, por lo que será más asequible encontrar soluciones al rondar los 2 millones de combinaciones, mientras que en la segunda, \textit{WorkSpace 1000\_50\_10}, el valor de $k$ pasa a ser 10, rondando las diez mil millones de combinaciones.

La primera instancia se ha utilizado tanto en los ejemplos mostrados en la Figura~\ref{fig:centros_medicos} como para ajustar los parámetros del algoritmo. Finalmente, en el apartado de comparativa
...
\begin{itemize}
    \item \textbf{Alpha ($\alpha$)}: Durante la inicialización de las soluciones \textit{greedy}, en cada paso se incorpora un elemento $x_i$ a la solución. El parámetro $1-\alpha$ determina el porcentaje de los mejores candidatos $x_i \in X, \; x_i \neq 1$ entre los cuales se selecciona.  
...
Se empieza ajustando el parámetro $lr$ del \textit{MAB}, ya que es necesario para que el aprendizaje reforzado sea efectivo. Un $lr$ muy bajo hace que los pesos no se modifiquen, y todas las acciones tengan probabilidades parecidas, mientras que uno alto hace que las acciones estén muy descompensadas y no se refleje la realidad del valor esperado de cada acción.

El valor final escogido ha sido $lr=0.1$, utilizando valores entre 0.01 y 2.

Una vez ajustado el \textit{learning rate}, se busca el $\beta$, tomando candidatos en el rango $[0,1]$ con los valores $0, 0.1, 0.2, \dots, 0.9, 1$.
...
En total, esto equivale a 220 ejecuciones simultáneas, lo que permite observar la evolución del algoritmo y disponer de un espacio muestral suficientemente grande para comparar su desempeño bajo distintos valores de $\beta$.

Los resultados se representan mediante \textit{diagramas de caja}, los cuales permiten visualizar de manera resumida la distribución de los datos. 
...
En la Figura~\ref{fig:beta} se puede ver que utilizar aprendizaje reforzado llega a aumentar en hasta un 20 \% el rendimiento del algoritmo. Con $\beta=1$ los resultados rondan en torno a una mediana de $10$ soluciones mientras que valores cercanos a $0$ rondan las $12$ soluciones. Por tanto, se ha decidido tomar el $\beta=0.1$
...
Se puede ver en la Figura~\ref{fig:alpha} que lo ideal es tomar valores de alpha entre $0.2$ y $0.8$, sin una diferencia clara entre los mismos. Por tanto, arbitrariamente se decide tomar el $\alpha=0.3$.

\section{Resultados de los artículos}

\subsection{Primera vista a los resultados previos}
\label{sec:articulo2}
Tomando como referencia las soluciones reportadas en el artículo \cite{k-Balanced_2}, se realizó una comparativa para verificar si el método \textit{GRASP-MAB} desarrollado en este TFM alcanzaba los mismos resultados. De este análisis se observó que, en algunos casos, se obtenían soluciones superiores a las allí presentadas, mientras que en otros los resultados fueron menos favorables.
...
Se puede entonces suponer que en los resultados del artículo se habría asignado uno de estos puntos de demanda a la instalación 20 y el otro a la 13, obteniendo así los valores publicados.
Sin embargo, también podría plantearse que, con la premisa de los resultados redondeados, la solución no sería óptima.
Si ambos puntos de demanda se hubiesen asignado a la instalación 13, se hubiera obtenido $f_2 = 231$ y $f_3 = 127$.
...
Esto no se puede decir de la segunda instancia en la Tabla~\ref{tab:ws1000_50_10}. Al multiplicar por 10,000 el número de combinaciones se hace inviable este método. Por ello, como referencia tanto para los resultados del \textit{GRASP} como del \textit{MAB-GRASP},
...
Es decir, la cantidad de soluciones encontradas no dominadas en $F'$ (el Frente de Pareto encontrado) por el Frente de Pareto exacto ($F$), divididas entre la cantidad total de soluciones en $F'$.
...
Coverage & 0.0784 & 0.0531 & 0.118 \\ \hline
...
Coverage & 0.3628 & 0.3255 & 0.172 \\ \hline
...
También desarrollar otro tipo de Aprendizaje Reforzado, ya que en este trabajo se han considerado el \textit{contexto} como variable de entrada al \textit{MAB}, pero se podrían considerar otras,
