...
En el artículo \cite{k-balanced_1}, como se mencionó en la introducción, se propone el problema de localización multiobjetivo de k-centros balanceado y 
se resuelve con el algorimto MOAKBCL.
...
En el trabajo presentado en \cite{k-Balanced_2}, se profundiza en el problema de localización multiobjetivo de $k$-centros balanceados, proponiendo un enfoque híbrido que combina GRASP, Oscilación Estratégica (SO, por sus siglas en inglés) y Path Relinking. El objetivo principal es mejorar la calidad de las soluciones y explorar de manera más efectiva el espacio de búsqueda, incluyendo tanto regiones factibles como no factibles.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Se evalúan los centros candidatos no seleccionados, calculando el valor del objetivo $f_1$ \ref{eq:f1} suponiendo cada uno de ellos dentro de la solución. Se ha seleccionado $f_1$ únicamente para no complicar
...
\begin{itemize}
    \item \textbf{Búsqueda local  de $f_1$ \ref{eq:f1}:} Minimiza la máxima distancia entre el conjunto de centros seleccionados y cada uno de los elementos de la población.
    \item \textbf{Búsqueda local de $f_2$ \ref{eq:f2}:} Reduce el número máximo de clientes asignados a los centros.
    \item \textbf{Búsqueda local de $f_3$ \ref{eq:f3}:} Disminuye la diferencia entre el centro más sobrecargado y el menos utilizado.
\end{itemize}
...
De esta manera, se conservan únicamente las soluciones que pertenezcan al frente de pareto actual. En caso de o haber obtenido una solución nueva del frente o una solución
más cercana a la anterior al frente, se vuelve a realizar una nueva búsqueda local con esta nueva solución.

Se ha de considerar la posibilidad de empate tanto para la búsqueda local $f_2$ como para la $f_3$. Podría darse el caso de poder redistribuir los empates a emplazamientos seleccionados
con menor demanda, con tal de mejorar estas funciones objetivo. No se ha implementado en este algoritmo al no darse el caso, pero se ha detectado en uno de los estudios previos situaciones en los que podrían haber
soluciones empeoradas al redondear las distancias (ver en \ref{sec:articulo2}).
...
\begin{itemize}
    ...
    Un contexto $(0,0,0)$ indicaría que la solución ya pertenece al frente. Un contexto de $(30,15,10)$ indicaría que la solución debería disminuir en 30 $f_1$ para entrar en el frente de pareto, o disminuir 15 $f_2$ o 10 $f_3$ para hacerlo.
...
\begin{algorithm}[H]
\caption*{Algoritmo MAB GRASP}
\begin{algorithmic}[1]
    \State \textbf{Definiciones:}
    \State $MAX\_ITS$: Número de iteraciones
    \State $W$: Pesos del MAB
    \State $F$: Frente de soluciones
    \State $X$: Solución actual
    \State $X'$: Nueva solución candidata
    \State $C$: Contexto extraído de la solución y el frente
    \State $R$: Recompensa asociada a la mejora de la solución
    
    \Statex

    \State Inicializar $W$, $F \gets \emptyset$
    \While{$"ITS < MAX\_ITS$}
        \State $X \gets \text{ConstruccionGreedyRandomizada}(\alpha)$

        \State $\text{ActualizarFrente}(X, F)$
        \State $mejora \gets \text{true}$

        \While{$mejora$}
            \State $C \gets \text{ExtraerContexto}(X, F)$
            \State $Brazo \gets \text{SeleccionarBrazoMAB}(W, C)$
            
            \State $X' \gets \text{BusquedaLocal}(X, Brazo)$
            
            \State $R \gets \text{EvaluarSolucion}(X', X, F)$
            \State $\text{ActualizarPesosMAB}(W, Brazo, R)$
            
            \If{$X'$ es una nueva solución del frente \textbf{o} mejora a $X$}
                \State $X \gets X'$
                \State $\text{ActualizarFrente}(X', F)$
                \State $mejora \gets \text{true}$
            \Else
...
Esto aumenta el rendimiento de unas 5 iteraciones por segundo hasta más de 50 (esto dependerá de los cores de la CPU del ordenador utilizado).

Por haber utilizado esto, hay que tener en cuenta que no se puede, sin reducir en gran medida la velocidad del algoritmo, hacer que el conjunto de instancias paralelizadas compartan
