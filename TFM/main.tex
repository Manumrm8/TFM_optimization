...
En este capítulo se introducen los \textbf{problemas de localización de instalaciones} (\textit{Facility Location Problems}), un campo clave de la optimización combinatoria que busca determinar las ubicaciones óptimas para infraestructuras. Se presentan los modelos clásicos como el \textbf{\textit{p-mediana}}, enfocado en minimizar la distancia total; el \textbf{\textit{p-center}}, que minimiza la distancia máxima para garantizar equidad; y los \textbf{problemas de cobertura} (\textit{LSCP} y \textit{MCLP}), que buscan maximizar el servicio con un número limitado de instalaciones.

La motivación principal del problema abordado es la necesidad de ir más allá de los objetivos tradicionales, centrándose en un enfoque multiobjetivo que equilibre la \textbf{cobertura}, la \textbf{capacidad} de las instalaciones y la \textbf{homogeneidad} en su uso. Se busca evitar tanto la falta de acceso en ciertas zonas como la saturación de los centros seleccionados.

Para abordar este desafío, se formaliza un \textbf{modelo matemático} con tres funciones objetivo, a partir de un conjunto de $n$ puntos de demanda y $m$ ubicaciones potenciales para construir $k$ instalaciones:

\begin{itemize}
    \item \textbf{Minimizar la distancia máxima }
    \item \textbf{Minimizar la carga máxima}
    \item \textbf{Minimizar el desequilibrio}
\end{itemize}

El problema resultante es de naturaleza \textbf{discreta}, \textbf{no lineal}, \textbf{combinatoria} (NP-difícil) y \textbf{multiobjetivo}. Debido a esta complejidad, los métodos exactos son computacionalmente inviables, lo que justifica el uso de metaheurísticas para encontrar un conjunto de soluciones eficientes que representen un buen compromiso entre los tres objetivos.
