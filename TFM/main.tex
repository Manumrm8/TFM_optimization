...
El Aprendizaje por Refuerzo (del inglés, \textit{Reinforcement Learning} o RL) es un área del aprendizaje automático inspirada en la psicología conductista. Se utiliza en problemas donde un agente debe tomar acciones en un entorno dinámico, con el objetivo de para maximizar una cierta recompensa.
...
El objetivo del RL no es encontrar la respuesta correcta, sino descubrir una estrategia óptima de actuación. Por esta razón, el RL se considera un problema de optimización de decisiones. El agente busca optimizar su comportamiento para lograr un objetivo a largo plazo, lo que lo convierte en una herramienta excepcionalmente potente para resolver problemas complejos en dominios como la robótica, la gestión de la cadena de suministro, las finanzas algorítmicas o hasta los mismos problemas de optimización.
...
\begin{itemize}
    \item \textbf{Agente}: Entidad que aprende y toma las decisiones. Puede ser un robot aprendiendo a caminar, un programa jugando al ajedrez o un sistema de gestión de tráfico urbano.
    \item \textbf{Entorno}: Es el mundo, real o simulado, donde se encuentra el agente y con el que interactúa. El agente únicamente puede tener control parcial sobre el entorno.
    \item \textbf{Estado ($s$)}: Descripción de la situación actual del entorno. Representa toda la información relevante que el agente puede necesitar en la toma de decisiones. El conjunto de todos los posibles estados se conoce como espacio de estados ($S$).
...
El proceso iterativo va desarrollando a lo largo de los tiempos $t=0, 1, 2, ...$. En cada paso $t$:
\begin{enumerate}
...
Se define como el retorno esperado cuando agente, partiendo del estado $s$, si sigue la política $\pi$. En terminos coloquiales, mide cómo de bueno es tomar cada una de las acciones dependiendo de la política tomada.
...
Conocida como \textbf{función Q}, se define como el retorno esperado al tomar la acción $a$, en el estado $s$, y a partir de ahí seguir la política $\pi$.
...
Para que un agente aprenda la política óptima, deberá equilibrar explotación y exploración para diseñar la estrategia:
\begin{itemize}
...
\end{itemize}
Si únicamente explota, se quedará atascado en un local.\\
Si solo explora, obtendrá un rendimiento bajo al no poder alcanzar nunca óptimos locales, ni si quiera el global (se podrá quedar cerca pero no indagará en el entorno cercano).\\
Esto se suele solventar siguiendo una \textbf{política $\epsilon$-greedy (épsilon-voraz)}, donde el agente elige la mejor acción conocida con probabilidad $1-\epsilon$ (explotación), y una acción aleatoria con probabilidad $\epsilon$ (exploración).
...
\begin{itemize}
    ...
    \item \textbf{Estado}: En este caso no habría estado, ya que no podemos inferir nada de cómo se encuentre la propia máquina.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Estado}: La posición, velocidad de las articulaciones, lecturas de sensores (cámaras, sensores de proximidad) etc. Cualquier cosa que pueda medir el propio robot.
...
Se utiliza hasta para juegos de estrategia complejos como el ajedrez, Go o videojuegos.
