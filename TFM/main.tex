...
\section{Modelo Matemático}
Sea $N={1,2,...,n}$ un conjunto de puntos de demanda y $M={1,2,...,m}$ el conjunto de ubicaciones para las potenciales instalaciones. \\
También definamos $d_{ij}$ la distancia entre el punto de demanda $i \in N$ y la instalación $j \in M$.\\
Asumimos que cada punto de demanda es suplido por su instalación más cercana. Por tanto, definimos $n_j$ la cantidad de puntos de demandas asignados a la instalación $j$.\\
Sea $k$ el número de instalaciones que se han de construir. Entonces definimos como $S$ la solución formada por esos $k$ puntos.

Dicho esto, las tres funciones objetivo a definir serán:
$$
\text{Minimizar} \quad f_1(S) = \max_{i \in N} \min_{j \in S} d_{ij}
$$
Es decir, el objetivo de la $f1$ será que la distancia del punto que queda más alejado de todas las instalaciones construidas sea la menor posible.
$$
\text{Minimizar} \quad f_2(S) = \max_{j \in S} n_j
$$
$f_2$ se utilizará para que las instalaciones queden lo menos sobrecargadas posibles.

$$
\text{Minimizar} \quad f_3(S) = \max_{j \in S, \, j' \in S, \, j \neq j'} |n_j - n_{j'}|
$$

Para que las instalaciones se utilicen de manera equitativa, y no ocurra el caso contrario, que una quede con poco uso.
$$
 \text{sujeto a} \quad |S| = k
$$
La solución ha de contener exactamente $k$ puntos de instalaciones.ç

Enunciado el problema, podemos deducir ya varias cosas:
\begin{itemize}
    \item Es un problema de optimización no lineal, ya que las tres funciones objetivo utilizan tanto el operador máximo como el mínimo, que no son lineales.
    \item También es de optimización discreta, ya que el espacio de soluciones es finito, y sus variables discretas (conjunto también de elementos finitos).
    \item Es un problema de optimización combinatorio (NP-difícil), ya que se trata de encontrar un subconjunto de $k$ elementos de $M$.
    \item Es un problema multiobjetivo, tenemos tres funciones objetivo, con equidad de importancia entre ellas.
\end{itemize}
Por todas las consideraciones anteriores, ni existen métodos exactos para resolverlo ni sería viable probar todas las soluciones, al ser NP-difícil. Por lo tanto hay que escoger 
métodos alternativos como los algoritmos heurísticos para poder resolverlo.
