...
def _calculate_f1(solution_indices, all_distances, k):
    """Calcula el máximo de las distancias mínimas."""
    return np.max(np.min(all_distances[:, solution_indices], axis=1))

def _calculate_f2(solution_indices, all_distances, k):
    """Calcula la carga máxima en cualquier instalación."""
    assignment_indices = np.argmin(all_distances[:, solution_indices], axis=1)
    if assignment_indices.size == 0: return 0 # Caso extremo sin demandas
    _, counts = np.unique(assignment_indices, return_counts=True)
    return np.max(counts)

def _calculate_f3(solution_indices, all_distances, k):
    """Calcula la diferencia entre la carga máxima y mínima."""
    assignment_indices = np.argmin(all_distances[:, solution_indices], axis=1)
    if assignment_indices.size == 0: return 0 # Caso extremo
    
    unique_indices, counts = np.unique(assignment_indices, return_counts=True)
    
    # Manejo de conteos cero
    all_counts = np.zeros(k, dtype=int)
    all_counts[unique_indices] = counts
    return np.max(all_counts) - np.min(all_counts)


# --- 2. El Motor de Búsqueda Local Genérico ---

# meter _calculate_f1 como parámetro, aunque no se si valdrá

def local_search_unified(initial_solution, df_distances_demand, k, m, n_veces, objective_calculator):
    """
    Realiza una búsqueda local optimizada para una función objetivo dada.

    Args:
        initial_solution (list): Solución inicial.
        df_distances_demand (pd.DataFrame): DataFrame de distancias.
        m (int): Número total de posibles instalaciones.
        n_veces (int): Límite de mejoras a realizar antes de detenerse.
        objective_calculator (function): La función que calcula el valor del objetivo 
                                         (ej: _calculate_f1).
    Returns:
        tuple: (mejor_solucion, mejor_valor_objetivo)
    """
    ...
    # Usar un set para búsquedas O(1), una pequeña optimización extra.
    current_solution_set = set(current_solution)
    
    current_objective = objective_calculator(current_solution, all_distances, k)
    ...
    while True:
        ...
        for i in range(k):
            # Guardamos el valor que vamos a sacar para actualizar el set eficientemente.
            s_out = current_solution[i]

            for j in range(m):
                if j in current_solution_set:
                    continue
                
                temp_solution = current_solution[:]
                temp_solution[i] = j
                
                temp_objective = objective_calculator(temp_solution, all_distances, k)
                
                if temp_objective < current_objective:
                    current_objective = temp_objective
                    current_solution = temp_solution
                    
                    # Actualizamos el set para la siguiente iteración
                    current_solution_set.remove(s_out)
                    current_solution_set.add(j)
